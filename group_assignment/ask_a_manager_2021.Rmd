...
ask_a_manager_2021 %>%
  ...
  mutate(percent = 100* n/sum(n)) %>% head()
...
favstats(ask_a_manager_2021$converted_salary) # find a really rich guy here
...
ask_a_manager_age %>% 
 ...
 # geom_point(aes(x = age_group, y=mean_salary_by_age, color = gender))+
  ...
  NULL
```
As we can see, the gender pay gap gets larger the older indiviudals get. In addition men and women start off at similar pays.
...
ask_a_manager_2021 %>% filter(highest_level_of_education_completed == "High School") %>% count()
...
ask_a_manager_salary <- ask_a_manager_2021 %>% 
  ...
  select(if_white,converted_salary)
ask_a_manager_salary
t.test(converted_salary~if_white, data = ask_a_manager_salary, success = "Yes")
```
Here we see that Non-whites have a higher mean salary on average. This is due to asian americans having an extremely high average salary.
...
prop.test(x = c(74,454), n = c(74+3786,454+20585), alternative = "two.sided")
```
...
prop.test(x = c(1152,6905), n = c(1152+2708,6905+14134), alternative = "two.sided")
```
...
So to conclude there is a significant difference in proportion of whites and non whites who acheive a masters degree.
...
meanmodel <- lm(converted_salary ~ 1, data=reg_data)
...
huxreg(meanmodel, m1, m2, m3, m4, m5,
